...
if (class_exists('WP_Customize_Control')) {
	if (!class_exists('honrix_Text_Radio_Button_Custom_Control')) {
		...
		class honrix_Text_Radio_Button_Custom_Control extends WP_Customize_Control
		{
			...
			{
				wp_enqueue_style('honrix-custom-controls-css', trailingslashit(get_template_directory_uri()) . 'inc/customize/assets/customizer.css', array(), '1.0', 'all');
			}
			/**
					...
					<div class="radio-buttons">
						...
						foreach ($this->choices as $key => $value) { ?>
							<label class="<?php echo esc_attr($class); ?>">
								...
								<span for="_customize-input-<?php echo esc_attr($this->id); ?>-radio-<?php echo esc_attr($value); ?>"><?php echo wp_kses_post($value); ?></span>
							</label>
						<?php	} ?>
					</div>
					<?php if (esc_attr($this->id) == 'honrix_blog_style' || esc_attr($this->id) == 'honrix_pagination_style') { ?>
